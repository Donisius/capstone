...
const App = () => {
  ...
  const stop = () => {
    camera.stop();
    // Reset canvas.
    canvasRef.current.getContext('2d').clearRect(0, 0, DISPLAY_SETTINGS.width, DISPLAY_SETTINGS.height);
  }
  ...
  return (
    <div className='wrapper'>
      ...
      <video
        style={{ display: 'none', height: DISPLAY_SETTINGS.height, width: DISPLAY_SETTINGS.width }}
        ref={videoRef}>
      </video>
      <canvas
        className='output-canvas'
        ref={canvasRef}
        style={{ height: DISPLAY_SETTINGS.height, width: DISPLAY_SETTINGS.width }}>
      </canvas>
    </div>
  );
}
